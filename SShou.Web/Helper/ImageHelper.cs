...
namespace SShou.Web.Helper
{
    public class ImageHelper
    {
        ...
        {
            FileInfo file = new FileInfo(waterPicPath);
        ...
        public static Bitmap Circular(string file, int width, int height)
        {
        ...
        public static void GeneratePoster(string savePath, string syPicPath, string waterPicPath, string headPicPath)
        {
            // 检测文件夹
            FileInfo fileSource = new FileInfo(waterPicPath);
            string direName = fileSource.DirectoryName;
            if (!Directory.Exists(direName)) Directory.CreateDirectory(direName);
            // 生成二维码缩略图
            string thumPath = direName + @"\thum_" + new FileInfo(syPicPath).Name;
            CutForCustom(File.OpenRead(syPicPath), thumPath, 100, 100, 100);
            // 生成圆形头像
            Bitmap bitmap = ImageHelper.Circular(headPicPath, 183, 183);
            // 合成图片
            System.Drawing.Image image = System.Drawing.Image.FromFile(savePath);
            System.Drawing.Image waterImage = System.Drawing.Image.FromFile(thumPath);
            System.Drawing.Graphics graphic = System.Drawing.Graphics.FromImage(image);
            int x = (image.Width - waterImage.Width) / 2;
            // int y = image.Height - waterImage.Height;
            int y = (image.Height - 295);
            graphic.DrawImage(waterImage, new System.Drawing.Rectangle(x, y, waterImage.Width, waterImage.Height), 0, 0, waterImage.Width, waterImage.Height, System.Drawing.GraphicsUnit.Pixel);
            graphic.DrawImage(bitmap, new System.Drawing.Rectangle((image.Width - bitmap.Width) / 2, (image.Height - 720), bitmap.Width, bitmap.Height), 0, 0, bitmap.Width, bitmap.Height, System.Drawing.GraphicsUnit.Pixel);
            graphic.Dispose();
        ...
        public static void GeneratePoster(string savePath, string syPicPath, string waterPicPath, string headPicPath, string text)
        {
            // 检测文件夹
            FileInfo fileSource = new FileInfo(waterPicPath);
            string direName = fileSource.DirectoryName;
            ...
            if (!string.IsNullOrWhiteSpace(text))
            {
                ...
                graphic.DrawString("我是第", font, brush, 100, 30);
                int len = 100 + (text.Length * 19);
                graphic.DrawString(text, font, new SolidBrush(Color.Red), len, 30);
                graphic.DrawString("名环保大使", font, brush, (len + 53), 30);
            }
            image.Save(waterPicPath);
            image.Dispose();
        }
